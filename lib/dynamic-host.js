...
return {
  ...
  apply(ctx) {
    ...
    function createEditorApi(ctx, sessions, agents, log = () => {}, hooks = {}) {
       ...
       */
      async function writeMarker(session, span, op, targetSeq, originalText, extra = {}) {
        ...
        return writer(session, span, { op, targetSeq, originalText, ...extra })
      }
      ...
      const recall = op(async (args) => {
        ...
        // 2026-09-07:撤回语义 = 遮蔽目标轮及之后全部(编辑=从此处分叉,bfb965e4/3f9e4f12
      ...
      /**
       * 折叠完成块(三段式窗口批1 marker 折叠承载):遮蔽 [start..end] 完成块区间,
       * 日志 append-only 不变(sourceEventSeqs 记录被遮蔽 seq,可展开)。摘要由批2
       * 生成(marker content);本 op = 承载(区间折叠 + marker 写入)。
       * 端点必须是当前 surface 节点;已被遮蔽的端点 → target-shadowed。
       */
      const fold = op(async (args) => {
        const sessionId = String(args?.sessionId ?? '')
        const session = requireSession(sessionId)
        await ensureIdle(agents.get(sessionId))
        const startSeq = Number(args?.start)
        const endSeq = Number(args?.end)
        if (!Number.isInteger(startSeq) || !Number.isInteger(endSeq) || startSeq < 0 || endSeq < startSeq) {
          throw editorError('bad-fold-range', 'fold 需要合法完成块区间 [start..end](surface 节点 seq,start<=end)')
        }
        // 主路径:index.js 注入 args.span(文件 spanFromFile range mode,官方 foldSurface nodes);
        // fallback:内存 surface.nodes 位置段(host 视图尽力)。
        const span = args?.span ?? rangeSpanFrom(session, startSeq, endSeq)
        if (!span) throw editorError('target-shadowed', '折叠区间端点不在当前 surface(已被遮蔽或非节点)——完成块边界需为 surface 节点')
        // 折叠 marker content 为空(批2 确定摘要填充);editor 记录区间供展开/审计
        // 折叠 marker content = 确定摘要(批2,index.js 从文件生成注入 args.summary;
        // 摘要替代折叠块进模型上下文——折叠语义 = 归档可回填,非移除)
        const summary = typeof args?.summary === 'string' && args.summary.trim() ? args.summary : ''
        const markerEvent = await writeMarker(session, span, 'fold', startSeq, '', {
    ...
    /** 内存 surface.nodes 的区间位置段(尽力;主路径 = 文件 spanFromFile range mode)。 */
    function rangeSpanFrom(session, startSeq, endSeq) {
      const nodes = Array.isArray(session?.surface?.nodes) ? session.surface.nodes : []
     ...
     *   ② 无打开 step 但有打开着的 turn（回合内 step 间隙，1e99e1ff 现场）
      ...
      return {
        ...
        async writeMarker(session, span, meta) {
          if (typeof session.snapshotEvents === 'function') {
            // 0.1.7 V4 admission 拒绝 kind:'plugin'（encode 失败→marker 批次滞留→下一轮
            // turn 崩 producer-owned source kind）；写官方迁移规范形状 plugin:retrace。
            const data = { id: editorId(meta.op), role: 'user', content: Array.isArray(meta.content) ? meta.content : [], source: { kind: 'plugin:retrace' }, editor: { targetSeq: meta.targetSeq, text: meta.originalText ?? '' }, shadowedSeqs: span.shadowedSeqs.slice() }
            const surfaceOp = { op: 'replace', startSeq: span.start, endSeq: span.end }
            const sourceEventSeqs = span.shadowedSeqs.slice()
            await validateMarker(session, { type: 'user/message', data, surfaceOp, sourceEventSeqs })
            return session.append('user/message', data, { surfaceOp, sourceEventSeqs })
          }
          ...
          const marker = {
            id: editorId(op),
            role: 'assistant',
            // fold 摘要进 content(替代折叠块进模型上下文);其余 op 空(replace 遮蔽派生无模型消息)
            content: Array.isArray(meta.content) ? meta.content : [],
          ...
          if (openStep) {
            ...
            wrappedAfter = [
              ...
              // 缺失 = malformed → 官方 validation 拒绝 → 会话加载失败（1f4d986e 事故，
              // 修复线 check-turn-end-reason.mjs 固化）。我们的信封 turn 立即完整关闭 →
    ...
    // 感知（1e99e1ff 同类风险，独立审查 2026-09-02 记录）；正式装配在
