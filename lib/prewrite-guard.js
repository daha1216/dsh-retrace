 ...
 */
import { editorError, sessionEvents } from './host-core.js'
...
export function createMarkerGuard({ log = () => {}, prewriterFactory, enabled = () => true, rollbackRatio = ROLLBACK_RATIO } = {}) {
  ...
  return {
    ...
    async validateMarkerAppend(session, envelope, extra = {}) {
      ...
      // 快照点守卫（2026-08-31 c2d05ce9 事故闭环）：回档请求（遮蔽过大）
      ...
      if (typeof session.snapshotEvents === 'function') {
        ...
        try {
          const probe = Session.fromRestore(session.id, session.snapshotEvents(), session.header, session.inheritedEventCount, 'shared-frozen')
          for (const event of extra.wrappedBefore ?? []) probe.append(event.type, event.data)
          probe.append(envelope.type, envelope.data, { surfaceOp: envelope.surfaceOp, sourceEventSeqs: envelope.sourceEventSeqs })
          for (const event of extra.wrappedAfter ?? []) probe.append(event.type, event.data)
          return { t1Ok: true }
        } catch (error) {
          throw editorError('marker-rejected', 'Official Session preflight failed: ' + error.message)
        }
      }
      if (factory === null) {
        try {
      ...
      let verdict
      try {
        const prewriter = factory({ events: sessionEvents(session) })
      ...
      const candidate = sessionEvents(session).slice()
