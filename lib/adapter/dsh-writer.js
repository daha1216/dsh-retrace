 ...
 */
import { editorError, editorId, lastModelSource, sessionEvents } from '../host-core.js'
    ...
    async writeMarker(session, span, meta) {
      if (typeof session.snapshotEvents === 'function') {
        const data = {
          ...
          // 0.1.7 起 V4 admission（session-format-v3-to-v4/message-sources.ts）拒绝
          // kind:'plugin'（retired plugin wrapper）：encode 失败 → marker 批次滞留 →
          // 下一轮 turn 崩「format v4 message requires a producer-owned source kind」。
          // 改写官方 V3→V4 迁移对未注册插件的规范产出形状 plugin:retrace；读侧
          // client 的 marker match 兼容新旧两种形状（旧 V3 文件由迁移自动改写）。
          source: { kind: 'plugin:retrace' },
          editor: { targetSeq: meta.targetSeq, text: meta.originalText ?? '' },
          shadowedSeqs: span.shadowedSeqs.slice(),
        }
        const surfaceOp = { op: 'replace', startSeq: span.start, endSeq: span.end }
        const sourceEventSeqs = span.shadowedSeqs.slice()
        if (typeof validateMarker !== 'function') throw editorError('writer-unavailable', 'Marker preflight is required')
      ...
      const surfaceOp = typeof session.snapshotEvents === 'function'
