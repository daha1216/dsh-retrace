 ...
 */
var FLOW_ROW_SCOPE = "[data-chat-flow-kind]";
var OUR_ROW_SCOPE = ".dsh-rt-user-row";
var OUR_SUBTREE_SCOPE = `${FLOW_ROW_SCOPE}, ${OUR_ROW_SCOPE}, .dsh-rt-editor`;
function isLayoutRelevant(records) {
  for (const record of records) {
    if (record.type === "childList") {
      if (record.target instanceof Element && (record.target.closest(OUR_ROW_SCOPE) !== null || record.target.matches(FLOW_ROW_SCOPE))) return true;
...
function installChatActionAligner(ctx) {
  ...
  observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["class", "style", "hidden"], characterData: true });
