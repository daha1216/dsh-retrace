window.__ModuleLoader__.load({id: "dsh-retrace", factory: (require) => { var module = { exports: {} }; var exports = module.exports;
...
// ../../../../.dsh/profiles/web/node_modules/dsh-retrace/lib/client.js
var client_exports = {};
...
// ../../../../.dsh/profiles/web/node_modules/dsh-retrace/lib/badge.js
function uuidOf(id) {
...
function UserActionsRow({ node, sessionId, useChat, inputActions, t }) {
  ...
  const confirmTimer = (0, import_react.useRef)(0);
  const textareaRef = (0, import_react.useRef)(null);
  (0, import_react.useEffect)(() => () => clearTimeout(confirmTimer.current), []);
...
function RecallMarkerRow({ node, useChat, t }) {
  ...
  const label = op === "recall" ? count > 1 ? t("marker.recallMany", { count }) : t("marker.recallOne") : op === "regenerate" ? t("marker.regenerate") : op === "fold" ? t("marker.fold", { count }) : t("marker.edit");
...
var READER_MOUNT_FLAG = "dshRtReader";
/**
 * 取当前会话的 chat 节点快照（ChatNodeStore：get / values / byKey）。
 * 走官方 conversation target 面：ctx.sessions.binding(id) → ctx.uiConversation.binding(binding).target("chat")。
 * 不能读 binding.session.getSnapshot().chat —— SessionSnapshot 没有 chat 字段（0.4.22 阅读页注入器静默失效的原因）。
 * chat target 只有激活后才有快照，注入器不靠订阅隐式激活，故显式 activate("chat")（会话生命周期内单调，重复调用无副作用）。
 * 任何一步拿不到就返回 null，注入器安静跳过。
 */
...
function readerChatInfo(ctx) {
  ...
  const nodes = readerChatNodes(ctx, currentId) ?? sessions?.binding?.(currentId)?.session?.getSnapshot?.()?.chat?.nodes;
  if (!nodes || typeof nodes.values !== "function") return null;
  const actionsBySeq = /* @__PURE__ */ new Map();
...
function mountReaderChipRow(t, sessionId, entry, row, cluster) {
  ...
  const settle = (op, result) => {
    ...
    if (op === "recall") {
      const echoed = typeof result.value?.text === "string" && result.value.text.length > 0 ? result.value.text : textOf(entry.content);
...
function scanReaderClusters(ctx, t) {
  ...
  for (const cluster of document.querySelectorAll("[data-reader-anchor][data-reader-key]")) {
    ...
    mountReaderChipRow(t, info.sessionId, { seq, messageId: String(action.messageId), content: node.data?.content }, row, cluster);
  }
}
function apply(ctx) {
