...
/* 对话页紧凑化：官方 chat.node 槽位把 user-actions 渲染成气泡下方的独立行
   ...
   alignChatActionRows 量出并写进 inline transform；收行与悬停显隐全走 CSS，
   React 在 chips ↔ 内联编辑器之间切换时能立刻恢复行高与官方行位置。 */
...
// shifts the official clock · copy row left by chips+gap (inline transform
// only) and parks the chips right after the shifted copy button — one
// continuous row inside the message column, right edge kept on the column
// edge. Geometry is measured, so clock width, branch/usage seats,
...
function alignChatActionRows() {
  ...
  for (const row of rows) {
    ...
    if (row.querySelector(".dsh-rt-user-actions") === null) { release(); continue; }
  ...
  for (const { row, anchor } of anchors) {
    ...
    if (prevShift !== undefined && prevShift.dx === dx && prevShift.dy === dy && row.style.transform === chatActionTransform({ dx, dy })) continue;
