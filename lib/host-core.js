...
export function sessionEvents(session) {
  ...
  throw editorError('session-contract', 'Session does not expose a supported event reader')
}
...
export function createEditorApi(ctx, sessions, agents, log = () => {}, hooks = {}) {
   ...
   * 找轮边界,不遍历稀疏的 sessionEvents(session) 数组(host events 可能只加载部分,
   ...
   * 轮边界,不遍历稀疏的 sessionEvents(session) 数组——host 的 events 可能只加载部分
  ...
  const editAndResend = op(async (args) => {
    ...
    const startSeq = fromScratch
      ? (sessionEvents(session).findIndex((e) => isRoundBoundary(e)) >= 0 ? sessionEvents(session).findIndex((e) => isRoundBoundary(e)) : 0)
      : seq
    ...
    // 2026-09-01 二次修复:若外部传入 span(host 从文件读全量算好,绕开稀疏 sessionEvents(session)),
  ...
  const regenerate = op(async (args) => {
    ...
    for (let i = idx - 1; i >= 0; i--) {
      if (isRoundBoundary(sessionEvents(session)[nodes[i]])) {
        userSeq = nodes[i]
        break
      }
    }
    if (userSeq === -1) throw editorError('no-prompt', 'No user message precedes this reply; cannot regenerate.')
    const text = extractUserText(sessionEvents(session)[userSeq]?.data?.content)
    if (!text.trim()) {
