...
// only writes an inline translate() onto our own row to park the chips left of
// the copy cluster. Geometry is measured, so clock width, branch/usage seats,
...
function chatActionShift(row) {
  ...
  // Hidden flow items (find-in-page folding) may sit between the two rows.
  ...
  const dx = Math.round((target.left - CHAT_ACTION_GAP - (own.right - (prevShift?.dx ?? 0))) * 100) / 100
  return { dx, dy }
}
...
function alignChatActionRows() {
  ...
  for (const row of rows) {
    ...
    const prevShift = row._dshRtShift
    if (prevShift !== undefined && prevShift.dx === shift.dx && prevShift.dy === shift.dy) continue
  ...
  for (const [row, shift] of writes) {
    ...
    if (shift === null) {
      ...
      row.style.transform = `translate(${shift.dx}px, ${shift.dy}px)`
