...
const recallMarkerDefinition = {
  ...
  match: (event) => {
    ...
    if (event.type === 'user/message' && event.data?.source?.plugin === 'retrace') return { id: `marker:${event.data.id}`, role: 'start' }
...
function UserActionsRow({ node, sessionId, useChat, inputActions, t }) {
  ...
  return createElement('div', { className: 'dsh-rt-user-row' }, [
    editing
      ? createElement('div', { key: 'editor', className: closing ? 'dsh-rt-editor dsh-rt-editor-closing' : 'dsh-rt-editor' }, [
...
const CSS = `
.dsh-rt-strip{display:inline-flex !important;align-items:center;gap:2px;opacity:1;visibility:visible}
...
/* 对话页紧凑化：官方 chat.node 槽位把 user-actions 渲染成气泡下方的独立行
   （flowItem），这里把该行收成 0 高、把槽位输出的 chips 行平移进官方复制按钮
   那一行（几何由 apply() 的 alignChatActionRows 量出并写进 inline transform）。收行
   与悬停显隐全部走 CSS，React 在 chips ↔ 内联编辑器之间切换时能立刻恢复行高。 */
[data-chat-flow-kind="user-actions"]:has(.dsh-rt-user-row>.dsh-rt-user-actions){height:0;min-height:0;margin-top:0 !important;overflow:visible}
...
/* chips 现在停在列右侧的页边空档里，那里横着 harness 的列宽拖拽条
   （asLM_W_widthHandle，absolute、z-index:8、pointer-events:auto，全高 40px）——它是
   定位元素且层级更高，会截走 chips 的点击（拖拽变成调列宽）。transform 会把
   .dsh-rt-user-row 变成 z-index:0 的层，所以层级必须加在这一行上；9 只压过拖拽条，
   仍低于 z-index:20 的 FmRkdG_overlayLayer（弹层容器），不会盖住菜单。 */
[data-chat-flow-kind="user-actions"] .dsh-rt-user-row{position:relative;z-index:9}
...
.dsh-rt-marker-block{display:flex;flex-direction:column;align-items:center;gap:4px;width:100%;max-width:var(--dsh-chat-content-width);box-sizing:border-box;margin:10px auto;padding:0;position:relative}
.dsh-rt-marker-block:not([data-dismissed])::before{content:"";position:absolute;left:0;right:0;top:11px;height:1px;background:var(--dsw-alias-border-l2);opacity:.6;pointer-events:none}
.dsh-rt-marker{position:relative;z-index:1;display:inline-flex;align-items:center;justify-content:center;color:var(--dsw-alias-label-tertiary);background:color-mix(in srgb,var(--dsw-alias-bg-base) 75%,transparent);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid color-mix(in srgb,var(--dsw-alias-border-l2) 60%,rgba(255,255,255,.2));border-radius:999px;padding:2px 12px;font-size:11px;line-height:16px;font-weight:500;box-shadow:0 1px 3px rgba(0,0,0,.03),inset 0 1px 0 rgba(255,255,255,.15);letter-spacing:.2px;transition:all .18s ease}
.dsh-rt-marker:hover{color:var(--dsw-alias-label-secondary);border-color:var(--dsw-alias-border-l1);box-shadow:0 2px 6px rgba(0,0,0,.06),inset 0 1px 0 rgba(255,255,255,.25)}
...
.dsh-rt-reference{width:min(540px,85%);box-sizing:border-box;backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);background:color-mix(in srgb,var(--dsw-alias-bg-elevated) 68%,transparent);border:1px solid color-mix(in srgb,var(--dsw-alias-border-l2) 60%,rgba(255,255,255,.2));border-left:3px solid var(--dsw-alias-state-business-primary);border-radius:12px;padding:5px 12px;box-shadow:0 2px 8px rgba(0,0,0,.03),inset 0 1px 0 rgba(255,255,255,.15);transition:all .18s cubic-bezier(0.16,1,0.3,1)}
.dsh-rt-reference:hover{border-color:var(--dsw-alias-border-l1);border-left-color:var(--dsw-alias-state-business-primary);box-shadow:0 4px 12px rgba(0,0,0,.05),inset 0 1px 0 rgba(255,255,255,.25)}
...
.dsh-rt-reference summary:before{content:"▸";display:inline-block;transition:transform .15s ease;font-size:10px;color:var(--dsw-alias-label-caption)}
.dsh-rt-reference[open] summary:before{transform:rotate(90deg);color:var(--dsw-alias-label-secondary)}
.dsh-rt-reference summary:hover{color:var(--dsw-alias-label-primary)}
.dsh-rt-reference summary:hover:before{color:var(--dsw-alias-label-primary)}
...
// ---------------------------------------------------------------------------
// Reader-view injector（dsh-better-display 阅读页）
// ---------------------------------------------------------------------------
// better-display 的阅读视图不渲染 conversation.chat.node 槽位（chips 由本注入器
// 补上）。数据与对话视图同源：官方 chat 节点快照里的 user / user-actions /
// recall-marker 节点。DOM 锚点只用 better-display 的稳定标记
// [data-reader-anchor][data-reader-key] 与 button[aria-label="复制消息"] 所在行，
// 不碰它的内部 class。挂载点：chips 进复制按钮所在行；编辑器/报错挂到消息簇末尾。
const READER_MOUNT_FLAG = 'dshRtReader'

/**
 * 取当前会话的 chat 节点快照（`ChatNodeStore`：get / values / byKey）。
 *
 * 走官方 conversation target 面：`ctx.sessions.binding(id)` → `SessionBinding`，
 * 再 `ctx.uiConversation.binding(binding).target('chat')`。这里不能读
 * `binding.session.getSnapshot()`——SessionSnapshot 只有 sessionId/queue/pending/
 * running/... 没有 `chat` 字段，取到的永远是 undefined（这正是 0.4.22 阅读页
 * 注入器静默失效的原因）。
 *
 * chat target 只有在激活后才会有快照（assembler.snapshot() 对未激活目标返回
 * undefined），而注入器不像阅读页 React 视图那样通过订阅隐式激活，所以这里显式
 * `activate('chat')`；激活对会话生命周期是单调的，重复调用无副作用。
 * 任何一步拿不到就返回 null，注入器安静跳过，绝不让页面报错。
 */
function readerChatNodes(ctx, sessionId) {
  ...
  try {
    ...
    return conversation.target?.('chat')?.getSnapshot?.()?.nodes ?? null
  } catch {
    return null
  }
}

function readerChatInfo(ctx) {
  const sessions = ctx.get?.('sessions')
  const currentId = sessions?.list?.getSnapshot?.()?.current
  if (!currentId) return null
  // 官方 target 面优先；同时保留旧的 session.chat 取数路径作为兜底（其他
  // harness 版本若暴露该字段仍可工作）。
  const nodes = readerChatNodes(ctx, currentId)
    ?? sessions?.binding?.(currentId)?.session?.getSnapshot?.()?.chat?.nodes
  if (!nodes || typeof nodes.values !== 'function') return null
  // 一次遍历建三张索引：撤回动作（seq→messageId）、影子集、用户节点（key→seq/content）
  const actionsBySeq = new Map()
  const shadowedSeqs = new Set()
  for (const node of nodes.values()) {
    if (node.kind === 'user-actions' && typeof node.data?.seq === 'number') {
      actionsBySeq.set(node.data.seq, node.data)
    } else if (node.kind === 'recall-marker' && Array.isArray(node.data?.shadowedSeqs)) {
      for (const seq of node.data.shadowedSeqs) shadowedSeqs.add(seq)
    }
  }
  return { sessionId: currentId, nodes, actionsBySeq, shadowedSeqs }
}

/** 挂一套 ghost chips（编辑/撤回 + 两步确认 + 内联编辑器）到阅读页一个用户消息簇。 */
function mountReaderChipRow(t, sessionId, entry, row, cluster) {
  let busy = false
  let armed = false
  let timer = 0
  let editorHost = null
  ...
  span.dataset[READER_MOUNT_FLAG] = '1'
  span.dataset.rtSeq = String(entry.seq)

  const editBtn = document.createElement('button')
  editBtn.type = 'button'
  editBtn.className = 'dsh-rt-ghost'
  editBtn.title = t('action.edit')
  editBtn.textContent = t('action.edit')

  const recallBtn = document.createElement('button')
  recallBtn.type = 'button'
  recallBtn.className = 'dsh-rt-ghost dsh-rt-ghost-danger'
  recallBtn.title = t('action.recallUser')
  recallBtn.textContent = t('action.recall')

  const errorRow = document.createElement('div')
  errorRow.className = 'dsh-rt-error'
  ...
  const settle = (op, result) => {
    busy = false
    editBtn.disabled = false
    recallBtn.disabled = false
    if (!result || result.ok !== true) {
      const code = result?.error?.code
      showError(code === 'agent-busy' ? t('error.busy') : (result?.error?.message ?? t('error.generic')))
      return
    }
    if (op === 'recall') {
      const echoed = typeof result.value?.text === 'string' && result.value.text.length > 0
  ...
  const run = (op, extra = {}) => {
    ...
    errorRow.style.display = 'none'
    callOp(op, { sessionId, messageId: entry.messageId, ...extra }).then(
      (result) => settle(op, result),
      (error) => settle(op, { ok: false, error }),
    )
  }
  const closeEditor = () => {
    if (editorHost !== null) {
      editorHost.classList.add('dsh-rt-editor-closing')
  ...
  const openEditor = () => {
    ...
    editorHost.className = 'dsh-rt-editor'
    const textarea = document.createElement('textarea')
    textarea.className = 'dsh-rt-textarea'
    textarea.setAttribute('aria-label', t('action.editAria'))
    textarea.setAttribute('placeholder', t('action.editPlaceholder'))
    textarea.value = textOf(entry.content)
    const adjustHeight = () => {
      textarea.style.height = 'auto'
      textarea.style.height = `${Math.min(Math.max(textarea.scrollHeight, 56), 360)}px`
    }
    ...
    textarea.addEventListener('keydown', (event) => {
      if (event.key === 'Escape') {
        ...
        if (text.length > 0 && !busy) {
          run('editAndResend', { text, fromScratch: getConfig().editFromScratch })
        }
      }
    })
    const buttons = document.createElement('div')
    buttons.className = 'dsh-rt-editor-buttons'
    const send = document.createElement('button')
    send.type = 'button'
    send.className = 'dsh-rt-editor-send'
    send.textContent = t('action.send')
    ...
    cancel.type = 'button'
    cancel.className = 'dsh-rt-editor-cancel'
    cancel.textContent = t('action.cancel')
    cancel.addEventListener('click', closeEditor)
    buttons.append(send, cancel)
    editorHost.append(textarea, buttons)
    cluster.append(editorHost)
    adjustHeight()
    textarea.focus()
  ...
  recallBtn.addEventListener('click', () => {
    ...
    armed = true
    recallBtn.className = 'dsh-rt-ghost dsh-rt-ghost-armed'
    recallBtn.textContent = t('action.recallConfirm')
    recallBtn.title = t('action.recallConfirmHint')
    timer = setTimeout(disarm, RECALL_CONFIRM_MS)
  })

  span.append(editBtn, recallBtn)
  row.append(span)
  cluster.append(errorRow)
  const cleanup = () => {
    clearTimeout(timer)
    span.remove()
    errorRow.remove()
  ...
  span._dshRtCleanup = cleanup
  return cleanup
}

/** 扫描阅读页 DOM：给无 chips 的用户簇挂载，摘除已消失/已被影子的挂载。 */
function scanReaderClusters(ctx, t) {
  const info = readerChatInfo(ctx)
  for (const span of document.querySelectorAll('[data-dsh-rt-reader="1"]')) {
    const cluster = span.closest('[data-reader-anchor]')
    const seq = Number(span.dataset.rtSeq)
    const shadowedNow = info !== null && Number.isFinite(seq) && info.shadowedSeqs.has(seq)
    if (!cluster || !cluster.isConnected || shadowedNow) {
      const cleanup = span._dshRtCleanup
      if (typeof cleanup === 'function') cleanup()
      else span.remove()
    }
  }
  if (info === null) return
  for (const cluster of document.querySelectorAll('[data-reader-anchor][data-reader-key]')) {
    if (cluster.querySelector('[data-dsh-rt-reader="1"]') !== null) continue
    const node = typeof info.nodes.get === 'function' ? info.nodes.get(cluster.dataset.readerKey) : null
    if (!node || (node.kind !== 'user' && node.kind !== 'steering')) continue
    const seq = node.data?.seq
    if (typeof seq !== 'number' || info.shadowedSeqs.has(seq)) continue
    const action = info.actionsBySeq.get(seq)
    if (!action || action.messageId === undefined) continue
    const copyBtn = cluster.querySelector('button[aria-label="复制消息"]')
    const row = copyBtn !== null && copyBtn.parentElement !== null ? copyBtn.parentElement : cluster
    mountReaderChipRow(t, info.sessionId,
      { seq, messageId: String(action.messageId), content: node.data?.content }, row, cluster)
  }
}
...
// Keep the chips clear of the window edge when the gutter is tight.
const CHAT_ACTION_EDGE = 8
...
/** The official copy button of a user flow item; null when only the cluster is known. */
function userFlowCopyButton(prev) {
  const labelled = prev.querySelector('button[aria-label="复制"], button[aria-label="复制成功"], button[aria-label="Copy"], button[aria-label="Copied"]')
  if (labelled !== null) return labelled
  const actions = userFlowActionRow(prev)
...
/**
 * Required horizontal/vertical offset for one chips row, or null when it must stay in flow.
 * Primary placement is immediately RIGHT of the copy button. The copy button sits at the
 * message column's right edge (zero room inside the column), so the chips ride into the page
 * gutter right of the column; the seat's flow row is zero-height with overflow:visible, which
 * makes that overflow paint instead of clip. `rightRoom < chips width` means the viewport has
 * no room at all right of the copy button: fall back to the left of the clock+copy cluster so
 * the row can never overlap the copy button nor leave the viewport.
 * A zero-size row (seat hidden with display:none by the shadow/marker hide plan) keeps its
 * previous shift — re-measuring an unlaid-out seat would park the chips at random.
 */
function chatActionShift(row) {
  const flow = row.closest('[data-chat-flow-kind]')
  if (flow === null || flow.getAttribute('data-chat-flow-kind') !== 'user-actions') return null
  if (row.querySelector('.dsh-rt-user-actions') === null) return null
  ...
  const anchor = userFlowActionRow(prev)
  if (anchor === null) return null
  const prevShift = row._dshRtShift
  const own = row.getBoundingClientRect()
  if (own.width === 0 && own.height === 0) return prevShift ?? null
  const target = anchor.getBoundingClientRect()
  const copy = userFlowCopyButton(prev)
  const copyRight = copy === null ? target.right : copy.getBoundingClientRect().right
  const dy = Math.round((target.top + target.height / 2 - (own.top - (prevShift?.dy ?? 0) + own.height / 2)) * 100) / 100
  const ownLeft = own.left - (prevShift?.dx ?? 0)
  const ownRight = own.right - (prevShift?.dx ?? 0)
  const rightRoom = window.innerWidth - CHAT_ACTION_EDGE - copyRight
  let dx
  if (rightRoom >= own.width) {
    // Right of the copy button; when the gutter is narrower than gap+chips, pinch the gap
    // down to 0 (never below copyRight) so a transiently wider row stays put.
    const left = Math.min(copyRight + CHAT_ACTION_GAP, window.innerWidth - CHAT_ACTION_EDGE - own.width)
    dx = Math.round((Math.max(left, copyRight) - ownLeft) * 100) / 100
  } else {
    dx = Math.round((target.left - CHAT_ACTION_GAP - ownRight) * 100) / 100
  }
  return { dx, dy }
}
...
/** Re-park every conversation-page chips row. Reads first, then writes (one layout flush). */
function alignChatActionRows() {
  const rows = document.querySelectorAll('[data-chat-flow-kind="user-actions"] .dsh-rt-user-row')
  const writes = []
  for (const row of rows) {
    const shift = chatActionShift(row)
    if (shift === null) {
      if (row._dshRtShift !== undefined) writes.push([row, null])
      continue
    }
    const prevShift = row._dshRtShift
    // Trust but verify: an equality check against the cached shift alone would skip the
    // rewrite when something else dropped the inline transform while the geometry stayed
    // the same, leaving the chips wherever the zero-height flow row put them.
    if (prevShift !== undefined && prevShift.dx === shift.dx && prevShift.dy === shift.dy
      && row.style.transform === chatActionTransform(shift)) continue
    writes.push([row, shift])
  }
  for (const [row, shift] of writes) {
    // Mutating only our own nodes' inline style: the components pass no style
    // prop, so React reconciliation never rewrites it.
    if (shift === null) {
      row.style.transform = ''
...
function installChatActionAligner(ctx) {
  ...
  // childList alone misses the layout changes that matter here: the reasoning
  // fold toggles `hidden`/content-visibility, hover mirrors land as class
  // changes and the armed chip swaps its label (and width) with no node added.
...
export function apply(ctx) {
  ...
  // Reader-view injector：better-display 阅读页不渲染 chat.node 槽位，用 DOM
  // 注入补 ghost chips。MutationObserver 用 rAF 去抖（流式输出时 DOM 变化密集）；
  // 定时扫描兜底覆盖无 DOM 变化的快照更新（撤回后影子判定变化等）。
  if (typeof document !== 'undefined' && typeof MutationObserver !== 'undefined') {
    let scheduled = false
    const scan = () => scanReaderClusters(ctx, t)
    const schedule = () => {
      if (scheduled) return
      scheduled = true
      requestAnimationFrame(() => {
